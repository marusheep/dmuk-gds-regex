CASE
    ...
    WHEN REGEXP_MATCH(Product Name, '(?i).*serum$|(?i).*mask.*|(?i).*essence.*|(?i).*cleanser$|(?i).*toner.*|(?i).*moisturiser.*|.*moisturizer.*|(?i).*night cream.*|(?i).*face.*|(?i).*eye.*|(?i).*crème.*|(?i).*cleansing oil$|(?i).*lotion$|(?i).*skin nutrition booster$|(?i).*purifying treatment bar.*|(?i).*brightening powder$') THEN "Skincare" 
    ELSE "Other"
END
