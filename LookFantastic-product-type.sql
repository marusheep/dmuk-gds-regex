CASE
    ...
    WHEN REGEXP_MATCH(Product Name,'.*(?i)Bath.*|.*(?i)Shower gel.*|.*(?i)Shower.*oil.*|.*(?i)Body Wash.*|.*(?i)Body Polish.*|.*(?i)Feminine Wash.*') 
        THEN "Bath/Body Wash"
    WHEN REGEXP_MATCH(Product Name,'.*(?i)Hand.*cream.*|.*(?i)Hand.*balm.*|.*(?i)Hand Wash.*|.*(?i)Hand.*spray.*') 
        THEN "Hand & Foot Care"

    WHEN REGEXP_MATCH(Product Name,'.*(?i)Make.*up remover.*|.*(?i)Make.*up removal.*|.*Bioderma.*Removing Solution.*') 
        THEN "Makeup Remover"
    WHEN REGEXP_MATCH(Product Name,'.*(?i)Mascara.*|.*(?i)Eye lash.*|.*(?i)Eye brow.*|.*(?i)Eye.*shadow.*|.*(?i)Medium Brown.*|.*(?i)Eye.*liner.*|.*(?i)Eye.*(?i)Pencil.*|.*(?i)Brow.*Pencil.*|.*(?i)Brow Shaper.*|.*(?i)Eyebrow Crayon.*|.*(?i)ink liner.*') 
        THEN "Eye Makeup"
    WHEN REGEXP_MATCH(Product Name,'.*(?i)Palette.*') 
        THEN "Palette"
    WHEN REGEXP_MATCH(Product Name,'.*(?i)Primer.*|.*(?i)Foundation.*|.*(?i)Concealer.*|.*(?i)Broner.*|.*(?i)Blush.*|.*(?i)Highlight.*|.*(?i)Powder.*|.*(?i)Fusion Cushion.*|.*(?i)BB Cream.*|.*(?i)CC cream.*|.*NYX.*Setting Spray.*|.*Vita.*(?i)Body Blur.*|.*(?i)Stila.*Correct.*|.*Yves Saint Laurent.*(?i)Moisture Glow.*|.*Bobbi Brown.*(?i)Face Base.*|.*Estee Lauder.*Double Wear.*Makeup.*') 
        THEN "Complexion"
    ...
    WHEN REGEXP_MATCH(Product Name,'.*(?i)Cleansing.*|.*(?i)Cleanser.*|.*(?i)Facial.*cleaner.*|.*(?i)Facial.*soap.*|.*(?i)facial.*wash.*|.*(?i)Face Wash.*|.*(?i)peel.*|.*(?i)Exfoliater.*|.*(?i)Exfoliant.*|.*(?i)Cleansing.*Milk.*|.*(?i)cosrx.*patch.*') 
        THEN "Peels/Face Cleanser"
    WHEN REGEXP_MATCH(Product Name,'.*(?i)lip.*balm.*|.*(?i)Lip.*scrub.*|.*(?i)Lip.*Protectant.*|.*(?i)Liquid.*Collagen.*') 
        THEN "Lip Care"
    ...
    WHEN REGEXP_MATCH(Product Name,'.*(?i)Mist.*|.*(?i)*balm.*|.*(?i)Thermal.*Water.*|.*Caudalie.*(?i)Grape Water.*|.*La Roche-Posay.*(?i)Serozinc.*') 
        THEN "Mist/Balm"
    WHEN REGEXP_MATCH(Product Name,'.*(?i)Essence.*|.*(?i)essentials.*|.*(?i)Ampoule.*|.*(?i)Serum.*|.*(?i)Capsules.*|.*(?i)Concentrate.*|.*(?i)Skin.*Lift.*|.*(?i)Treatment Gel.*|.*(?i)Ecological Compound.*|.*Estee Lauder.*(?i)Advanced Night Repair.*Recovery Complex.*|.*Lancôme.*Genifique.*|.*(?i)The Ordinary.*Caffeine.*Solution.*|.*The Ordinary.*Formula.*') 
        THEN "Essence"
    WHEN REGEXP_MATCH(Product Name,'.*(?i)Lotion.*|.*(?i)Cream.*|.*(?i)Crème.*|.*(?i)Emulsion.*|.*Clinique.*Moisturising Gel.*|.*(?i)Moisturiser.*') 
        THEN "Facial Cream"
    WHEN REGEXP_MATCH(Product Name,'.*(?i)Toner.*|.*(?i)Tonic.*|.*(?i)Confort Tonique.*|.*Erno Laszlo.*Skin Supplement.*') 
        THEN "Toner"
